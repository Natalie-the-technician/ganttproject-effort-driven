...
private class DurationInputs(
  ...
  val effort: Double?,
  /**
   * [fork change] Both axes: who contributes how many hours and when they are away (axis B), and
   * whose absence stops the whole day (axis A). One value, so that a caller cannot take the one
   * and forget the other -- see [EffortInputs].
   */
  val effortInputs: EffortInputs,
  /**
   * [fork change] The working-day test OF THIS TASK, built from the working weeks of the people on
   * it and remembering what it has already answered.
   *
   * PER TASK AND NOT PER RUN. Since `arbeitswoche-wirkung` the answer to "is this a working day"
   * depends on who is on the task, so it belongs to the task, next to the task's other inputs, and
   * not next to the calendar.
   */
  val isWorkingDay: (LocalDate) -> Boolean,
...
fun durationAtStart(
  ...
  return fabrik@{ levelTask, start ->
    ...
    // [fork change] BOTH AT ONCE, and neither of them optional: the days off travel in
    // `inputs.effortInputs` (both axes, see [EffortInputs]) and the working-day test is the one
    // built FOR THIS TASK out of the working weeks of its people. Handing the project calendar in
    // here instead would leave the arithmetic right and the grid wrong.
    daysNeededWithDaysOff(effort, inputs.effortInputs, start, inputs.isWorkingDay)
