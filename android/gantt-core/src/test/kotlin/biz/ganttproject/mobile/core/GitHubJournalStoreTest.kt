 ...
 *
 * The cases that carry the design are the last three: what happens when
 * somebody else wrote in between, what happens on a save that changes
 * nothing, and what happens to a failure. Everything above them is the
 * groundwork those three stand on.
 */
class GitHubJournalStoreTest {
  ...
  private fun storeOn(
    ...
    github,
    GitHubTarget("natalie", "stunden", path, branch),
    withToken
  )
  ...
  fun `load on an empty repository is null and not a failure`() {
    ...
    // The distinction the whole first run depends on: nothing there is not
    // the same as could not look. Reporting a failure here would have the app
    // complain on the day it is first installed.
  ...
  fun `load returns the stored text byte for byte`() {
    // Every trap in one string: umlauts and eszett (two bytes each in UTF-8),
    // an emoji (four, a surrogate pair in Kotlin), the field separator of the
    // journal format, and a trailing newline that a careless trim would eat.
    ...
    // Byte-for-byte, not merely equal-looking: a charset slip further up
    // produces a string that prints the same and hashes differently, and the
    // journal's check file is a SHA-256 over exactly these bytes.
    assertArrayEqualsBytes(
      journal.toByteArray(Charsets.UTF_8),
      loaded!!.toByteArray(Charsets.UTF_8)
    )
  }

  @Test
  fun `load copes with the line breaks GitHub puts in its base64`() {
    // Long enough to be wrapped several times. The basic decoder throws on a
    // line break inside the payload; only the MIME decoder skips it. Nothing
    // about a short journal would reveal that.
  ...
  fun `a directory where the journal should be is a failure, not an empty journal`() {
    ...
    val store = storeOn(github)
    assertNull(store.load())
    // Null again, because the interface has no other channel — but the
    // failure is recorded, and no baseline was taken, so the save that
    // follows cannot create a file over the directory's name.
  ...
  fun `save changes the file when there is one, naming the version it replaces`() {
    ...
    // An update carries the sha, and it is the one from the read — see the
    // conflict test for why that matters more than it looks.
    assertTrue(github.requests.last().body!!.contains(jsonString(shaWhenRead!!)))
  }
  ...
  fun `revision carries the commit, and a second save carries a different one`() {
    ...
    // Two saves, two commits. A blob sha would also differ here because the
    // content did, which is exactly why this test alone does not prove the
    // choice — the next one does.
  ...
  fun `revision is the commit and not the blob sha of the content`() {
    ...
    // The distinction that decides what a restore can do. The blob sha names
    // content only: two identical journals saved weeks apart share one, and
    // it cannot say when, by whom, or what else moved with it. The commit
    // names a point in the history, which is what `?ref=` and `git show`
    // take.
    assertFalse(
  ...
  fun `the commit message arrives as given, tabs newlines and quotes included`() {
    ...
    val store = storeOn(github)
    store.load()
    // The change list is built from real notes, and a note holds whatever
    // somebody typed. An unescaped newline makes the request body
    // unparseable at the far end, and the journal then silently stops saving.
  ...
  fun `the same text twice is Unchanged and costs no request at all`() {
    ...
    // Counted, not asserted. One GET for the load, one PUT for the save, and
    // then nothing: the second save neither commits nor looks.
  ...
  fun `a foreign write between load and save is refused, not overwritten`() {
    ...
    // The whole point, and the same answer as #2818 on the WebDAV side:
    // refuse, do not overwrite. Their content is still there.
  ...
  fun `a refused save stays refused until the journal is read again`() {
    ...
    // A caller that simply tries again must not succeed by trying. This is
    // the trap in the obvious repair: dropping the stale sha after a
    // conflict, so the retry fetches a fresh one, turns a refusal into the
    // overwrite it was there to prevent.
    ...
    // Reading again is the deliberate act that makes progress possible: now
    // the caller has seen the other version and can merge it.
  ...
  fun `saving without ever reading refuses to replace a file it has not seen`() {
    ...
    // No load() first. GitHub would accept a read-then-write done here, and
    // the other device's records would be gone with nothing to merge from.
  ...
  fun `a transport failure becomes a reason instead of escaping`() {
    ...
    val store = storeOn(github)
    store.load()
    // No network is the ordinary state of a phone, not an exceptional one.
    // An exception escaping here is a crash in the middle of saving work.
  ...
  fun `a failed load takes no baseline, so the save after it cannot overwrite`() {
    ...
    // The trap this guards: load() returned null because it could not look,
    // and a store that recorded "there is nothing there" would let the next
    // save create a file over one it never saw.
